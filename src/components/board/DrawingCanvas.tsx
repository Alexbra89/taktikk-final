...
import { drawPaint, drawingColorKey } from './drawTools';
...
const ArrowHead: React.FC<{
  ...
  return (
    <polygon points={`${end.x},${end.y} ${x1},${y1} ${x2},${y2}`} style={{ fill: paint }} opacity={opacity} />
  );
};
...
// Formen sjekkes derfor her også, og en tegning uten palettfarge får den
// nærmeste – aldri en usynlig strek.
// Fargen settes i style, ikke som attributt: var() virker ikke i SVG-attributter.
...
export const DrawingCanvas: React.FC<DrawingCanvasProps> = ({ drawing }) => {
  const paint = drawPaint(drawingColorKey(drawing));
  const line = { stroke: paint };
  const area = { stroke: paint, fill: paint };
  ...
  switch (drawing.type) {
    case 'circle': {
      ...
      return <circle cx={start.x} cy={start.y} r={r} style={area} {...AREA} />;
    }
    ...
    case 'label': {
      ...
      return (
        <text x={at.x} y={at.y} textAnchor="middle" dominantBaseline="middle"
          ...
          style={{ fill: paint, stroke: 'rgb(var(--k-pitch))', paintOrder: 'stroke', pointerEvents: 'none' }}>
          {text}
        </text>
      );
    }
  ...
  switch (drawing.type) {
    case 'arrow':
      return (
        <g>
          <line x1={first.x} y1={first.y} x2={last.x} y2={last.y} style={line} {...STROKE} />
          <ArrowHead start={first} end={last} paint={paint} opacity={0.88} />
        </g>
      );
      ...
      return (
        <g>
          <path d={`M ${first.x} ${first.y} Q ${c.x} ${c.y} ${last.x} ${last.y}`}
            style={line} fill="none" {...STROKE} />
          {/* Kurvens retning i endepunktet går fra kontrollpunktet. */}
          <ArrowHead start={c} end={last} paint={paint} opacity={0.88} />
        </g>
      );
    }
    ...
    case 'dashed':
      return (
        <line x1={first.x} y1={first.y} x2={last.x} y2={last.y}
          style={line} strokeDasharray="10,9" {...STROKE} />
      );
