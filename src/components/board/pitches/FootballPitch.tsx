...
// Kalk: analytikerens tavle. Kalklinjer på nesten svart (kveld) eller
// lys kalkgrå (dagslys) – ingen gressmønster, ingen vignett.
// Fargen kommer fra --k-pitch-line, så begge temaene følger med.

export const FootballPitch: React.FC = () => {
  const m = 32;
  const W = VW, H = VH;
  const line = 'var(--k-pitch-line)';
  const sw = 1.5;
  ...
  return (
    <g style={{ stroke: line }} strokeWidth={sw} fill="none">
      {/* Ytre ramme */}
      ...
      <circle cx={W / 2} cy={H / 2} r={3} style={{ fill: line }} />
      ...
      {/* Straffepunkter */}
      <circle cx={m + 86} cy={H / 2} r={2.5} style={{ fill: line }} />
      <circle cx={W - m - 86} cy={H / 2} r={2.5} style={{ fill: line }} />
      ...
      {/* Mål venstre */}
      <rect x={m - 22} y={H / 2 - 36} width={22} height={72} strokeWidth={1} />

      {/* Mål høyre */}
      <rect x={W - m} y={H / 2 - 36} width={22} height={72} strokeWidth={1} />
