...
export const PlayerManager: React.FC = () => {
  ...
  const create = () => {
    ...
    addPlayerAccount({
      ...
      password: playerPassword.trim() || pin,
      playerId: playerId || `player-${Date.now()}`,
      team: 'home',
    });
    setName(''); setPin(''); setEmail(''); setPlayerPassword(''); setRole(''); setPlayerId('');
  };
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
        ...
        <div className="bg-[#0f1a2a] rounded-2xl border border-[#1e3050] p-4">
          ...
          {/* E-post (valgfritt) */}
          <div className="mb-3">
            <SmLabel>E-post (valgfritt — for innlogging)</SmLabel>
            <input value={email} onChange={e => setEmail(e.target.value)}
              type="email" placeholder="spiller@epost.no"
              className="pm-inp" />
          </div>

          {/* Passord (valgfritt, brukes med e-post) */}
          <div className="mb-3">
            <SmLabel>Passord (valgfritt — for e-post innlogging)</SmLabel>
            <input value={playerPassword} onChange={e => setPlayerPassword(e.target.value)}
              type="password" placeholder="Standard: samme som PIN"
              className="pm-inp" />
          </div>
